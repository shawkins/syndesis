...
//    private final List<DdlAnalyzerException> exceptions = new ArrayList<DdlAnalyzerException>();
    ...
    protected void parseAndValidate() {
        ...
        if( !isOk(this.analyzer.checkAllParens()) ) {
            this.analyzer.addException(new DdlAnalyzerException("All parenthesis DO NOT MATCH"));
        ...
        if( numTokens < 4 ) {
            Position startPosition = new Position(getToken(0).beginLine, getToken(0).beginColumn);
            Position endPosition = new Position(getToken(2).endLine, getToken(2).endColumn+1);
            DdlAnalyzerException exception = 
                    new DdlAnalyzerException(
                            DiagnosticSeverity.Error,
...
//        Token cursorToken = this.analyzer.getTokenFor(position);
//        int cursorTokenIndex = getTokenIndex(cursorToken);
...
//
//        if( cursorToken.kind == AS) {
//            return new TokenContext(position, null, CONTEXT.AS_TOKEN);
//        }
