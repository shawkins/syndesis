...
//        if( tableBodylastIndex - tableBodyFirstIndex <= 1 ) {
//            Position startPosition = new Position(getTokens()[tableBodyFirstIndex].beginLine, getTokens()[tableBodyFirstIndex].beginColumn);
//            Position endPosition = new Position(getTokens()[tableBodylastIndex].endLine, getTokens()[tableBodylastIndex].endColumn);
                ...
                switch(count) {
                    ...
                    case 1: {
                        ...
                        if( tkn.kind == ID || tkn.kind == STRINGVAL) {
                            ...
                            Position startPosition = new Position(tkn.beginLine, tkn.beginColumn);
                            Position endPosition = new Position(tkn.endLine, tkn.endColumn+1);
                            DdlAnalyzerException exception = 
                                    new DdlAnalyzerException(
                                            DiagnosticSeverity.Error,
    ...
    protected TokenContext getTokenContext(Position position) {
        ...
        if( isInElement ) {
            ...
            if( tkn.kind == COMMA
                    ...
                    && getTokenIndex(tkn) == getLastTknIndex()) return null;
